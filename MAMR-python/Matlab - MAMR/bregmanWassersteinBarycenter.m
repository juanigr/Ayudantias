function [c,MVP,objectives] = bregmanWassersteinBarycenter(C,M,MaxCPU,lambda,useGPU,toleranceDifference,weights)
% INPUT:
...
NextPrint = 0;cpu = 0; k=0; Kn   = round(sqrt(d)); 
%for k=1:kMax
while cpu<=MaxCPU
     k = k+1;
     cpu = toc(t0);
     if cpu >= NextPrint %(mod(k, kPrint) == 0)
         differ=sum(std(UKv,1,2));
         c = mean(UKv,2);
         fprintf(1,'k = %5.0f, differ = %5.2e \n',compt,differ);
         imagesc(reshape(1-c,Kn,Kn));
         %title(strcat('k=',num2str(k),', val=',num2str(val),', t=',num2str(round(toc(t0)))));
         title(strcat('k=',num2str(k),', t=',num2str(NextPrint)));
         colormap hot;   
         name = strcat('Fig\',num2str(NextPrint),'-IBP.png'); 
         colormap hot;
         salvaPNG(gcf,name)
...
c = mean(UKv,2);
imagesc(reshape(1-c,Kn,Kn));
title(strcat('k=',num2str(k),', t=',num2str(round(toc(t0)))));
colormap hot;   
name = strcat('Fig\',num2str(NextPrint),'-IBP.png'); 
colormap hot;
salvaPNG(gcf,name)
