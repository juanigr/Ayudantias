...
def MAMR(
    ...
    """Ejecuta el algoritmo MAMR traducido desde MATLAB.

    ``d[m]`` debe tener dimensiones ``(R, S[m])`` y ``q[m]`` debe tener
    longitud ``S[m]``. Si no se entrega ``get_plan``, se utiliza internamente
    la traducción exacta de la función MATLAB ``GetPlan``.
    ...
    while cpu <= MaxCPU:
        ...
        if cpu >= next_print:
            ...
            if save_figures:
                _save_image(
                    p, kn, iteration, next_print, "MAMR", output_dir, xp
                )
            if nx_value <= tol:
                break
        ...
        for m in range(M):
            ...
            theta[m] = xp.maximum(theta[m] + beta - d[m], -beta)
    ...
    if save_figures:
        _save_image(
            p, kn, iteration, round(cpu), "MAMR", output_dir, xp
        )
